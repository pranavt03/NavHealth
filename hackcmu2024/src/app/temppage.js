...
import { Stethoscope, FileText, Calendar, ChevronDown, ChevronUp, User, X, Pill, Activity } from 'lucide-react';
...
export default function ClinicDashboard() {
  ...
  const stopRecording = () => {
    if (mediaRecorderRef.current) {
      ...
      if (audioURL) {
        uploadAndTranscribe(audioURL);
      }
    }
  };

  const uploadAndTranscribe = async (fileURL) => {
    setUploading(true);
    try {
      ...
      // Ensure that the audioBlob is in WAV format
      const formData = new FormData();
      formData.append('file', audioBlob, 'recording.wav'); // Set the file name with .wav extension
    ...
    <div className="flex h-screen bg-black text-green-400 font-mono">
      ...
      <div className="flex-1 overflow-auto p-6 bg-gradient-to-br from-gray-900 to-black">
        ...
        <div className="mb-6 bg-gray-800 p-4 rounded-lg border border-green-500 shadow-neon">
          ...
          {uploading && <p className="text-yellow-400">Uploading and transcribing...</p>}
          {transcription && (
            <div className="mt-4">
