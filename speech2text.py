...
if metric_key:
    ...
    if 'date' in df.columns:
        ...
        fig.update_layout(
            title='Glucose Readings',  # Set the title of the graph
            plot_bgcolor='rgba(0,0,0,0)',
            paper_bgcolor='rgba(0,0,0,0)',
            font_color='#00FF00',
            xaxis_title='Date',  # X-axis label
            yaxis_title='Glucose Level'  # Y-axis label
        )
...
# Function to generate glucose readings
def generate_glucose_readings(start_date="2023-05-01", num_readings=4):
    glucose_readings = []
    start = datetime.strptime(start_date, "%Y-%m-%d")
    for i in range(num_readings):
        date = (start + timedelta(days=i*15)).strftime("%Y-%m-%d")  # Every 15 days
        glucose = random.randint(120, 200)  # Generate random glucose levels between 120 and 200
        glucose_readings.append({"date": date, "glucose": glucose})
...
if st.sidebar.button("Add Patient"):
    if add_patient_name:
        if add_patient_name not in patients:
            # Add new patient with default values and autogenerated glucose readings
            patients[add_patient_name] = {
                "vitals": {"Heart Rate": 0, "Blood Pressure": "", "Temperature": ""},
                "medications": [],
                "diagnoses": [],
                "notes": [],
                "glucose_readings": generate_glucose_readings()  # Add default glucose readings
            }
